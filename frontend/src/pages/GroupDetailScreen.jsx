...
const GroupDetailScreen = () => {
  ...
  if (!group && notMember) return (
    <div className="max-w-4xl mx-auto px-8 pt-24 text-center space-y-4">
      ...
      <Link to="/groups" className="mc-label hover:opacity-70 transition-opacity inline-block">← Back to groups</Link>
    </div>
  );
    ...
    <div className="space-y-12 pb-20 px-8 pt-8 max-w-4xl mx-auto">
      ...
      {telemetry && (
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: 'Focus Density', value: `${telemetry.focusDensity}/day`, sub: 'sessions per member (7d)' },
            { label: 'Mean Session', value: `${telemetry.meanSession}m`, sub: 'avg duration (7d)' },
            { label: 'Total Sessions', value: telemetry.totalSessions, sub: 'last 7 days' },
          ].map(({ label, value, sub }) => (
            <div key={label} className="p-5 rounded-[20px] space-y-1" style={{ background: "oklch(var(--surface))" }}>
              ...
              <p className="mc-label italic">{sub}</p>
            </div>
          ))}
        </section>
      )}
          ...
          <div className="fixed bottom-20 right-0 sm:right-6 z-50 w-full sm:w-96 rounded-t-[28px] sm:rounded-[28px] border shadow-2xl flex flex-col overflow-hidden"
            ...
            <form onSubmit={handleAddNote} className="px-6 py-4 border-t flex gap-3" style={{ borderColor: "oklch(var(--text) / 0.06)" }}>
              <input type="text" value={noteInput} onChange={e => setNoteInput(e.target.value)}
                ...
                className="flex-1 px-4 py-2.5 rounded-full mc-body text-sm outline-none border"
